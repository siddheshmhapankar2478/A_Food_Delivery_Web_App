            ...
            return (
                  <div key={index}>
                  {
                  ...
                  }
                    {
                      (!items)?(null):
                        (items.map((item) => {
                                  const {imageId, name,description, id, price} = item?.card?.info;
                          if(imageId&&price){
                            isRendered = true;
                            return(
                              <div key={id} className='w-3/4 flex  mx-auto my-1 justify-between shadow-md '>
                                <div className='p-4 w-4/5'>
                                  <h1 className='text-xl font-bold p-1'>{name}</h1>
                                  <h1 className='font-bold p-1'>₹{price/100}</h1>
                                  <h1 className='p-1'>{description}</h1>
                                  <button className="border font-bold text-white bg-black p-2 border-black hover:bg-white hover:text-black rounded-sm" onClick={()=>{handleAddItems(item)}}>Add Item</button>
                                </div>
                                <div className='flex  my-auto mx-4 w-40 h-24'>
                                  <img className='w-48 h-24 rounded-lg' src={MENU_IMAGE_ITEM_URL+item?.card.info.imageId}/>
                                </div>
                            </div>);
                          }
                        }))
